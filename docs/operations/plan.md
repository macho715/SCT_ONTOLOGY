...
현재 구현 상태(2026-05-13): Cloudflare production MCP는 6개 tool을 노출한다. `ask_hvdc_ontology`는 데이터 전용이고, `render_hvdc_answer_card`가 `ui://hvdc/answer-card-v7.html` 카드 template를 소유한다. v6, v5, render-name resource alias는 ChatGPT 캐시/호환성 보호용으로 유지한다.
