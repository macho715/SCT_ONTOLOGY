...
| compose\_grounded\_answer | sessionId, evidenceIds, graphPathIds | GroundedAnswer | Answer Card |
...
`uiTemplate` metadata belongs to `render_hvdc_answer_card`. Data tools such as `ask_hvdc_ontology` and `search_ontology_corpus` return structured evidence data without directly attaching the iframe.

If the ChatGPT client cannot load the card template, the app must show a text fallback with `dataStatus=OK`, `uiRenderStatus=TEMPLATE_FETCH_FAILED`, `businessResultVisible=true`, and `fallbackUsed=true`. This warning is UI-only and must not change the business answer verdict or evidence.

## 8.2 Tool Descriptor Sketch

**MCP tool descriptor concept**

registerAppTool(server, "search\_ontology\_corpus", {
 title: "Search HVDC ontology corpus",
 description: "Search approved HVDC ontology documents and return evidence snippets for a specific logistics question. Do not use for public web search.",
 inputSchema: z.object({
 query: z.string(),
 requiredDocs: z.array(z.string()).optional(),
 domainHints: z.array(z.enum(["master","warehouse","document","marine","cost","material","port","communication","operations","team"])).optional(),
 topK: z.number().min(1).max(20).default(8)
 }),
 outputSchema: EvidenceSearchResultSchema,
 \_meta: {"\_meta.ui.resourceUri": "ui://hvdc/evidence-drawer.html"}
});
