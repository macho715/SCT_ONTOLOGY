...

{
 ...
 "\_meta": {"uiTemplate":"ui://hvdc/answer-card-v4.html", "piiMasked":true}
}

`uiTemplate` metadata belongs to `render_hvdc_answer_card`. Data tools such as `ask_hvdc_ontology` and `search_ontology_corpus` return structured data without directly attaching the iframe.
