...
- UI resource URI는 `ui://hvdc/answer-card.html`이다.
...
flowchart TD
  ...
  ExtApps --> WidgetResource["ui://hvdc/answer-card.html"]
...
`server/src/index.ts`는 `public/hvdc-answer-widget.html`을 읽어서 `ui://hvdc/answer-card.html` resource로 등록한다.

- resource 등록은 `registerAppResource`가 담당한다.
- resource MIME type은 `RESOURCE_MIME_TYPE`을 사용한다.
- tool descriptor의 `_meta.openai/outputTemplate`도 `ui://hvdc/answer-card.html`을 가리킨다.
...
public widget은 `ui://hvdc/answer-card.html` resource로 연결된다.
