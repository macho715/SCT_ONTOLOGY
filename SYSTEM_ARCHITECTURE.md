...
- ChatGPT는 App tool descriptor와 `ui://hvdc/answer-card-v8.html` resource metadata를 사용한다. `v7/v6/v5`는 stale ChatGPT session 호환 alias로 유지한다.
...
- UI resource URI는 `ui://hvdc/answer-card-v8.html`이다.
...
flowchart TD
  ...
  ExtApps --> WidgetResource["ui://hvdc/answer-card-v8.html"]
...
| `render_hvdc_answer_card` | Cloudflare: `server/src/hvdc-server.ts`<br/>Legacy local: `server/src/claude-render.ts` | `ui://hvdc/answer-card-v8.html` iframe 위젯 | Cloudflare MCP text/structured result 또는 legacy markdown fallback |
...
`server/src/hvdc-server.ts`는 generated widget module을 읽어서 `ui://hvdc/answer-card-v8.html` resource로 등록한다.

- resource 등록은 `registerAppResource`가 담당한다.
- resource MIME type은 `RESOURCE_MIME_TYPE`을 사용한다.
- `ask_hvdc_ontology`는 답변 JSON과 텍스트 fallback을 반환하고, ChatGPT 카드 표시를 위해 tool/result metadata로 `ui://hvdc/answer-card-v8.html`을 가리킨다.
- `ask_hvdc_ontology`의 `structuredContent`에는 `ui` 객체를 넣지 않는다. `ui.templateUrl`은 render tool에서만 붙인다.
- `render_hvdc_answer_card` descriptor도 `_meta.ui.resourceUri`와 `_meta["openai/outputTemplate"]`으로 `ui://hvdc/answer-card-v8.html`을 가리킨다.
- 사용자에게 보이는 최종 HVDC 답변은 `ask_hvdc_ontology` 한 번으로 카드 표시까지 시도한다. `render_hvdc_answer_card`는 이미 준비된 답변을 명시적으로 다시 렌더링할 때 사용한다.
- 호환성 alias resource는 `ui://hvdc/answer-card-v7.html`, `ui://hvdc/answer-card-v6.html`, `ui://hvdc/answer-card-v5.html`, `ui://hvdc/render_hvdc_answer_card.html`이다.
...
- **ChatGPT 레이어**: `server/src/worker.ts`가 Cloudflare Workers에서 `/mcp`를 열고, `server/src/hvdc-server.ts`가 `@modelcontextprotocol/ext-apps`의 `registerAppTool`로 ChatGPT App tool descriptor를 등록한다. `render_hvdc_answer_card`는 `ui://hvdc/answer-card-v8.html` iframe 위젯을 연결한다.
