...
def run_all_rules_canonical(
    df_items: pd.DataFrame,
    std_rate_table: Dict[str, float],
    hs_prefixes: List[str],
    required_certs: List[str],
    context: Dict[str, Any] = None,
) -> Dict[str, Any]:
    """Run existing rules and adapt the result for future MCP public output.

    Existing local API behavior remains unchanged through `run_all_rules`.
    This helper is the read-only semantic boundary for callers that need
    canonical HVDC evidence output.
    """
    from hvdc_semantic_adapter import canonicalize_rule_results

    local_result = run_all_rules(df_items, std_rate_table, hs_prefixes, required_certs)
    return canonicalize_rule_results(local_result, context or {})
