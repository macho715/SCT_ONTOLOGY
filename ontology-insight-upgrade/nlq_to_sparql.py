...
def canonicalize_nlq_query_result(nlq: str, rows: List[Dict[str, any]]) -> Dict[str, any]:
    """Convert supported local NLQ/SPARQL result rows into canonical read-only output.

    This does not execute SPARQL and does not expose an MCP tool. It only gives
    future public callers a stable adapter boundary after local query execution.
    """
    ...
    intent = detect_intent(nlq)
    if not rows:
        return no_evidence_result(intent, "SPARQL returned no rows.")

    if intent == "list_hvdc_codes":
        return adapt_case_timeline(
            {
                "sourceQuery": intent,
                "caseNumber": rows[0].get("caseNumber") or rows[0].get("code"),
                "events": [],
            }
        )

    if intent == "invoice_risk_analysis":
        first = rows[0]
        alerts = [
            {
                "riskType": risk_type,
            ...
            for risk_type in str(first.get("riskTypes", "")).split(", ")
            if risk_type
        ]
        return adapt_invoice_risk(
            {
                ...
                "invoice": {
                    ...
                    "totalAmount": first.get("totalAmount"),
                    "currency": first.get("currency"),
                    "supplierName": first.get("supplierName"),
                },
                "alerts": alerts,
            }
        )

    if intent in {"cost_analysis", "hs_code_risk", "container_stowage_analysis"}:
        return adapt_invoice_risk(
            {
                "sourceQuery": intent,
                "invoice": {
                    "invoiceNumber": rows[0].get("invoiceNumber"),
                    "hvdcCode": rows[0].get("caseNumber") or rows[0].get("code"),
                },
                "alerts": [
                    {
                        "riskType": intent.upper(),
                        "severity": rows[0].get("severity", "WARN"),
                    }
                ],
            }
        )
    ...
    if intent == "boe_cipl_mismatch":
        return adapt_invoice_risk(
            {
                "sourceQuery": intent,
                "invoice": {"invoiceNumber": rows[0].get("invoiceNo")},
                "alerts": [
                    {
                        "riskType": rows[0].get("mismatchType", "DOCUMENT_MISMATCH"),
                        "severity": "WARN",
                    }
                ],
            }
    ...
    return no_evidence_result(intent, f"No canonical adapter is registered for intent {intent}.")
