...
- The any-key resolver is read-only and returns evidence excerpts, not raw source rows by default.
- Resolver output masks local file paths, email addresses, token-like strings, secret-key-like strings, private links where detected, and Cloudflare-style 32-character account identifiers.
- Resolver output includes a `privacy` block that states whether masking was applied and confirms that raw evidence was not exposed.
- Resolver results do not mutate shipment, invoice, warehouse, graph, audit, email, approval, or payment state.
