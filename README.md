...
- `render_hvdc_answer_card`: 이미 준비된 `ask_hvdc_ontology` 결과를 명시적으로 다시 카드로 렌더링할 때 사용합니다. ChatGPT 서버는 `ui://hvdc/answer-card-v7.html` 카드 UI로, Claude 서버는 마크다운 카드로 출력합니다.
